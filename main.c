...
int main(void)
{
    ...
    char CAN_RECV_DATA[255];
    uint8_t CAN_MSG[8];
    uint32_t SampleValue=0;
    uint8_t CMD_RESPID=0;
    uint32_t SampleRecv=0xFFFFFF;
    uint32_t lop=0;
    ...
    while(1)
    {
        if(UARTHasData())
        {
            ...
            switch(strtoul(UARTStrGet(),NULL,0))
            {
                ...
                case icmdFlashGetData: //Read Version
                   UARTStrPut("Requesting store flash memory sample from sensor module. \r\n");
                ...
                case icmdFlashGenCSV:
                    //Code to generate a CSV line by line off of flash data.
                    GlobalTimer=0;
                    ...
                    {
                           Flash_Data = *((uint32_t *)lop);
                           sprintf(CSV_Line,"%d,%d\r\n",GlobalTimer,Flash_Data);
                           GlobalTimer++;
                    }
                    UARTStrPut("\r\n\r\n\r\n CSV END:\r\n");
                    break;
            ...
            switch(CMD_RESPID)
            {
                ...
                case icmdFlashGetData:
                    if(SampleRecv ==0xFFFFFF)
                    {
                        ...
                        if(SampleValue==0)//received end sample.
                        {
                            SampleRecv=0xFFFFFF;
                            sprintf(CAN_RECV_DATA,"Sample Received.\r\n");
                            UARTStrPut(CAN_RECV_DATA);
                        }else{
                            if((SampleRecv & 0x7ff)==0x400)
                            {
                                FlashErase(SampleRecv); //Erase 0x400 black at a time;
                            }
